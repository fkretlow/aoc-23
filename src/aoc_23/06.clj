...
;; For any given input race the following equation holds when we
...
;;
;; Thus we can compute the succesful button press durations by solving
;; the roots of
;;
;;   -t_b^2 + t*t_b - d_rec = 0
;;
;; which is easily done with the quadratic formula, and rounding both
;; solutions up to the next greater integer (strictly greater for the 
;; lower bound).
...
(defn- calculate-winning-range [t d_rec]
  (let [discriminant (- (* t t) (* 4 d_rec))]
    (when-not (neg? discriminant)
      [(next-greater-int (/ (+ (- t) (sqrt discriminant)) (- 2)))
       (int (ceil (/ (- (- t) (sqrt discriminant)) (- 2))))])))
